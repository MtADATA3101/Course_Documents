Yes---
title: "Data3101_Figures_Graphics"
author:
- Douglas A. Campbell
output:
  html_document:
    df_print: paged
    code_folding: hide
    keep_md: yes
    fig_caption: yes
    toc: TRUE
    toc_float: TRUE
bibliography_software: ROS_Phytoplankton.bib
bibliography_normal: ROS_Phytoplankton.bib
csl: plos-one.csl

---

```{r libraries}
#only install packages once
#install.packages("devtools")
#install.packages("tidyverse")
#install.packages("DiagrammeR")

#load package libraries
library(tidyverse)
library(DiagrammeR)
```
```{r data wrangling table}
DataWrangleTable <- tibble::tibble(Approach = c("Manual", "Interactive", "Interactive", "Scripted", "AI"),
                                
                                Reproducible = c("No", "Possible", "Yes", "Yes", "Possible"),
                                StartUp = c("Quick", "Quick", "Moderate", "Moderate", "Slow"),
                                Scale = c("Tiny", "Small", "Middle", "Big", "Bigger"),
                                Example = c("GoogleSheet", "OpenRefine", "RNotebook", "RScript", "DataIku"))

knitr::kable(DataWrangleTable, caption = "Data Wrangling Approaches")
saveRDS(object = knitr::kable(DataWrangleTable), file = file.path("RGraphics", "DataWranglingTable.Rds"))
```

```{r trial flowchart}
#https://datascienceplus.com/how-to-build-a-simple-flowchart-with-r-diagrammer-package/

TrialFlowChart <- DiagrammeR::grViz("digraph flowchart {
      # node definitions with substituted label text
      node [fontname = Helvetica, shape = rectangle]        
      tab1 [label = '@@1']
      tab2 [label = '@@2']
      tab3 [label = '@@3']
      tab4 [label = '@@4']
      tab5 [label = '@@5']

      # edge definitions with the node IDs
      tab1 -> tab2;
      tab2 -> tab3;
      tab2 -> tab4 -> tab5
      }

      [1]: 'Questionnaire sent to n=1000 participants'
      [2]: 'Participants came to clinic for evaluation n=700'
      [3]: 'Participants non-eligible for the study n=100'
      [4]: 'Participants eligible for the study n=600'
      [5]: 'Study sample n=600'
      ")

TrialFlowChart

saveRDS(object = TrialFlowChart, file = file.path("RGraphics", "TrialFlowChart.Rds"))
#readRDS(file = file.path("RGraphics", "TrialFlowChart.Rds"))

```

